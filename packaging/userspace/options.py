...
from compat_shims import Sequence, Type
...
class RunArgGroup(ArgparseActor):
	...
	def __set_name__(self, owner, name: str):
		...
		self.private_name = '_argparse_group_' + name
...
# TODO: move this to compat_shims
class RunOptsContainerMeta(ABCMeta):
	if sys.version_info < (3, 6):
		def __new__(mcls, name, bases, namespace):  # pylint: disable=W0221
...
class RunOptsContainer(metaclass=RunOptsContainerMeta):
	...
	def __new__(cls):
		...
		cls._groupsdict = _groupsdict
		cls._argsdict = _argsdict
		cls._optsdict = _optsdict
		cls._actordict = _actordict
